                                         ...
                                         int no_rebind)
{
    ...
    if ((l = calloc(1, sizeof(alistener))) == 0)
        goto nomem;
    if ((l->local_name = strdup(local_name)) == 0)
        goto nomem;
    if ((l->connect_to = strdup(connect_to)) == 0)
        goto nomem;
...
void start_device_log(void)
{
    ...
    strftime(path, sizeof(path),
             "/data/adb/adb-%Y-%m-%d-%H-%M-%S.txt", &now);
...
int launch_server(int server_port)
{
    ...
    /* wait for the "OK\n" message */
    {
        ...
        if (!ret) {
            fprintf(stderr,
                    "could not read ok from ADB Server, error = %ld\n",
                    GetLastError());
            return -1;
        }
        if (count != 3 || temp[0] != 'O' || temp[1] != 'K'
            || temp[2] != '\n') {
    ...
    if (pid == 0) {
        ...
        // child process
        int result =
            execl(path, "adb", "-P", str_port, "fork-server", "server",
                  NULL);
        ...
        if (ret < 0) {
            fprintf(stderr,
                    "could not read ok from ADB Server, errno = %d\n",
                    saved_errno);
            return -1;
        }
        if (ret != 3 || temp[0] != 'O' || temp[1] != 'K'
            || temp[2] != '\n') {
            fprintf(stderr, "ADB server didn't ACK\n");
            return -1;
        }
...
int adb_main(int is_daemon, int server_port)
{
    ...
    if (should_drop_privileges()) {
         ...
         */
        gid_t groups[] =
            { AID_ADB, AID_LOG, AID_INPUT, AID_INET, AID_GRAPHICS,
...
void connect_device(char *host, char *buffer, int buffer_size)
{
    ...
    if (fd < 0) {
        snprintf(buffer, buffer_size, "unable to connect to %s:%d", host,
                 port);
        return;
    }
                        ...
                        int reply_fd, asocket * s)
{
    ...
    if (!strncmp(service, "transport", strlen("transport"))) {
        ...
        if (!strncmp(service, "transport-usb", strlen("transport-usb"))) {
            ...
            if (!strncmp
                (service, "transport-local", strlen("transport-local"))) {
            type = kTransportLocal;
        } else
            if (!strncmp
                (service, "transport-any", strlen("transport-any"))) {
            type = kTransportAny;
        ...
        transport =
            acquire_one_transport(CS_ANY, type, serial, &error_string);
    ...
    if (!strncmp(service, "connect:", 8)) {
        ...
        snprintf(buf, sizeof(buf), "OKAY%04x%s", (unsigned) strlen(buffer),
                 buffer);
        writex(reply_fd, buf, strlen(buf));
        return 0;
    }
    ...
    if (!strcmp(service, "version")) {
        ...
        snprintf(buf, sizeof buf, "OKAY%04x%s", (unsigned) strlen(version),
                 version);
        writex(reply_fd, buf, strlen(buf));
        return 0;
    }
    ...
    if (!strncmp(service, "get-serialno", strlen("get-serialno"))) {
        ...
        snprintf(buf, sizeof buf, "OKAY%04x%s", (unsigned) strlen(out),
                 out);
        writex(reply_fd, buf, strlen(buf));
        return 0;
    }
    ...
    if (!strncmp(service, "get-state", strlen("get-state"))) {
        ...
        snprintf(buf, sizeof buf, "OKAY%04x%s", (unsigned) strlen(state),
                 state);
        writex(reply_fd, buf, strlen(buf));
        return 0;
    }

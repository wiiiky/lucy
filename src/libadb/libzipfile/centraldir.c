...
read_central_dir_values(Zipfile * file, const unsigned char *buf,
                        int len) 
{
...
#if 0
        printf
        ("file->buf=%p entry->data=%p dataOffset=%x localHeaderRelOffset=%d "
          "entry->fileNameLength=%d extraFieldLength=%d\n", file->buf,
         entry->data, dataOffset, localHeaderRelOffset,

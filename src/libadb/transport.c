...
static void dump_hex(const unsigned char *ptr, size_t len)
{
    ...
    char buffer[MAX_DUMP_HEX_LEN * 2 + 1 + MAX_DUMP_HEX_LEN + 1], *pb =
        buffer;

    if (len2 > MAX_DUMP_HEX_LEN)
...
static int
device_tracker_send(device_tracker * tracker, const char *buffer, int len)
{
...
asocket *create_device_tracker(void)
{
    ...
    if (tracker == 0)
        fatal("cannot allocate device tracker");
...
static void transport_registration_func(int _fd, unsigned ev, void *data)
{
    ...
    if (t->connection_state != CS_NOPERM) {
        ...
        fdevent_install(&(t->transport_fde),
                        t->transport_socket, transport_socket_events, t);
...
static int qual_match(const char *to_test,
                      const char *prefix, const char *qual,
                      int sanitize_qual)
{
    if (!to_test || !*to_test)

...
static void local_socket_event_func(int fd, unsigned ev, void *_s)
{
    ...
    if (ev & FDE_READ) {
        ...
        while (avail > 0) {
            ...
            D("LS(%d): post adb_read(fd=%d,...) r=%d (errno=%d) avail=%d\n", (int) s->id, (int) s->fd, (int) r, (int) (r < 0 ? errno : 0), (int) avail);
            if (r > 0) {
                avail -= r;
                x += r;
                continue;
...
static void remote_socket_close(asocket * s)
{
    ...
    send_packet(p, s->transport);
    D("RS(%d): closed\n", s->id);
    remove_transport_disconnect(s->transport,
...
PREFIX("usb:"),
        PREFIX("product:"), PREFIX("model:"), PREFIX("device:"),};
...
static int smart_socket_enqueue(asocket * s, apacket * p)
{
    ...
    if (service) {
         ...
         ** is clean up.
         */
        if (handle_host_request(service, ttype, serial, s->peer->fd, s) ==
            0) {
            /* XXX fail message? */

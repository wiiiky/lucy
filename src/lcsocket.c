                                     ...
                                     GCancellable * cancellable)
{
    ...
    GError *error=NULL;
    gboolean ret = g_socket_connect(G_SOCKET(socket), address, NULL, &error);
    if(error){
                                          ...
                                          GCancellable * cancellable)
{
    ...
    while ((n=lc_socket_receive(socket, buf, sizeof(buf))) > 0 && length>0) {
        g_string_append_len (string,buf,n);
        length-=n;
    }
    if(length!=0){
        goto ERROR;

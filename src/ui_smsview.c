...
{
    return ui_sms_view_construct(UI_SMS_TYPE_VIEW);
}
...
static void ui_sms_view_sms_paned_init(UISMSView * self)
{
    ...
    gtk_stack_add_named(GTK_STACK(self), GTK_WIDGET(priv->sms_paned),
                        STACK_NAME_SMS);

    priv->sms_grid = (GtkGrid *) gtk_grid_new();
    g_object_ref_sink(priv->sms_grid);
    gtk_paned_pack1(priv->sms_paned, GTK_WIDGET(priv->sms_paned), TRUE,
                    FALSE);
...
(GBaseFinalizeFunc) NULL, (GClassInitFunc) ui_sms_view_class_init, (GClassFinalizeFunc) NULL, NULL,
sizeof(UISMSView), 0, (GInstanceInitFunc) ui_sms_view_instance_init, NULL };

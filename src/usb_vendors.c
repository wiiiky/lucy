...
void usb_vendors_init(void)
{
    if (VENDOR_COUNT_MAX < BUILT_IN_VENDOR_COUNT) {
        fprintf(stderr,
                "VENDOR_COUNT_MAX not big enough for built-in vendor list.\n");
        exit(2);
    }
    ...
    if (get_adb_usb_ini(temp, sizeof(temp)) == 0) {
        ...
        if (f != NULL) {
            ...
            while (fgets(temp, sizeof(temp), f) != NULL) {
                ...
                if (errno == EINVAL || errno == ERANGE || value > INT_MAX
                    || value < 0) {
...
int build_path(char *buff, size_t len, const char *format,
               const char *home)
{
    if (snprintf(buff, len, format, home, ANDROID_PATH, ANDROID_ADB_INI) >=
        (signed) len) {
        return 1;
    }

...
int adb_get_emulator_console_port(void)
{
    ...
    if (serial == NULL) {
        ...
        while (*p) {
            ...
            if (!memcmp
                (p, LOCAL_CLIENT_PREFIX,
                 sizeof(LOCAL_CLIENT_PREFIX) - 1)) {
        ...
        if (memcmp
            (serial, LOCAL_CLIENT_PREFIX,
             sizeof(LOCAL_CLIENT_PREFIX) - 1) != 0)
...
static int switch_socket_transport(int fd)
{
    ...
    if (__adb_serial)
        snprintf(service, sizeof service, "host:transport:%s",
                 __adb_serial);
    else {
        char *transport_type = "???";

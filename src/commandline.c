...
static char *product_file(const char *extra)
{
    ...
    snprintf(x, (size_t) n, "%s" OS_PATH_SEPARATOR_STR "%s",
             gProductOutPath, extra);
    return x;
}
...
static void format_host_command(char *buffer, size_t buflen,
                                const char *command, transport_type ttype,
                                const char *serial)
{
    if (serial) {
...
static void status_window(transport_type ttype, const char *serial)
{
    ...
    format_host_command(command, sizeof command, "get-state", ttype,
                        serial);
...
int ppp(int argc, char **argv)
{
    ...
    if (fd < 0) {
        fprintf(stderr,
                "Error: Could not open adb service: %s. Error: %s\n",
...
static int send_shellcommand(transport_type transport, char *serial,
                             char *buf)
{
    int fd, ret;
...
static int logcat(transport_type transport, char *serial, int argc,
                  char **argv)
{
    char buf[4096];
...
//TODO: if we have a way to indicate debug, look in out/debug/target/...
    snprintf(path_buf, sizeof(path_buf),
             ...
             "product" OS_PATH_SEPARATOR_STR "%s", top_buf, hint);
...
int adb_commandline(int argc, char **argv)
{
  ...
  top:
    ...
    if (!strncmp(argv[0], "wait-for-", strlen("wait-for-"))) {
        char *service = argv[0];
        if (!strncmp
            (service, "wait-for-device", strlen("wait-for-device"))) {
            if (ttype == kTransportUsb) {
    ...
    if (!strcmp(argv[0], "forward")) {
        ...
        // Implement forward --remove <local>
            ...
            if (argc != 2)
                return usage();
            snprintf(buf, sizeof buf, "%s:killforward:%s", host_prefix,
                     argv[1]);
        }
        // Or implement one of:
        ...
        //    forward --no-rebind <local> <remote>
            ...
            if (argc != 3)
                return usage();
            const char *command =
                no_rebind ? "forward:norebind:" : "forward";
            snprintf(buf, sizeof buf, "%s:%s:%s;%s", host_prefix, command,
                     argv[1], argv[2]);
        }
...
static int delete_file(transport_type transport, char *serial,
                       char *filename)
{
    char buf[4096];
    char *quoted;
                ...
                char **argv)
{
    ...
    if (verification_file != NULL) {
        snprintf(verification_dest, sizeof(verification_dest), where,
                 get_basename(verification_file));

        if (!strcmp(apk_dest, verification_dest)) {
            fprintf(stderr,
                    "APK and verification file can't have the same name\n");
            return 1;
        }
    }
    ...
    if (verification_file != NULL) {
        err =
            do_sync_push(verification_file, verification_dest,
                         0 /* no verify APK */ );
        if (err) {
            goto cleanup_apk;
        } else {

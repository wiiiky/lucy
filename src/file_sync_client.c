...
static void END()
{
    ...
    fprintf(stderr, "%lld KB/s (%lld bytes in %lld.%03llds)\n",
            ...
            (long long) total_bytes, (t / 1000000LL),
            (t % 1000000LL) / 1000LL);
}
...
int sync_ls(int fd, const char *path, sync_ls_cb func, void *cookie)
{
    ...
    if (writex(fd, &msg.req, sizeof(msg.req)) || writex(fd, path, len)) {
        goto fail;
    }
    ...
    for (;;) {
        ...
        func(ltohl(msg.dent.mode),
             ltohl(msg.dent.size), ltohl(msg.dent.time), buf, cookie);
    }
                     ...
                     const char *name, int isdir)
{
    ...
    ci->dst = ci->src + ssize;
    snprintf((char *) ci->src, ssize, isdir ? "%s%s/" : "%s%s", spath,
             name);
    snprintf((char *) ci->dst, dsize, isdir ? "%s%s/" : "%s%s", dpath,
             name);
...
static int copy_local_dir_remote(int fd, const char *lpath,
                                 const char *rpath, int checktimestamps,
                                 int listonly)
{
    copyinfo *filelist = 0;
    copyinfo *ci, *next;
...
int do_sync_push(const char *lpath, const char *rpath, int verifyApk)
{
    ...
    if (S_ISDIR(st.st_mode)) {
        ...
        BEGIN();
        if (sync_send
            (fd, lpath, rpath, st.st_mtime, st.st_mode, verifyApk)) {
            return 1;
        } else {
            END();
            sync_quit(fd);
                      ...
                      const char *name, void *cookie)
{
    sync_ls_build_list_cb_args *args =
        (sync_ls_build_list_cb_args *) cookie;
    copyinfo *ci;
                             ...
                             const char *rpath, const char *lpath)
{
    ...
    while (dirlist != NULL) {
        ...
        if (remote_build_list
            (syncfd, filelist, dirlist->src, dirlist->dst)) {
            return 1;
        }
        free(dirlist);
        dirlist = next;
...
static int copy_remote_dir_local(int fd, const char *rpath,
                                 const char *lpath, int checktimestamps)
{
    copyinfo *filelist = 0;
    copyinfo *ci, *next;
    int pulled = 0;

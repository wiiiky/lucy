...
static void fdevent_subproc_event_func(int fd, unsigned events,
                                       void *userdata);
...
static void fdevent_register(fdevent * fde)
{
    ...
    if (fde->fd >= fd_table_max) {
        ...
        memset(fd_table + oldmax, 0,
               sizeof(int) * (fd_table_max - oldmax));
    }
...
{
    fdevent_set(fde,
                (fde->state & FDE_EVENTMASK) | (events & FDE_EVENTMASK));
}
